...
$kpiupf=mysqli_query($con,"SELECT * FROM kpidata ORDER BY DATA_ID ASC");
$kpidowf=mysqli_query($con,"SELECT * FROM kpidata ORDER BY DATA_ID ASC");
?>
...
window.onload = function () {
	var chart = new CanvasJS.Chart("chartContainer", {
		...
		data: [
		{
			...
			toolTipContent: "<p style='color:green'> {label}</p><hr/>Download Speed: {y} mb/s",
			color: "purple" ,
			dataPoints: [
                                <?php while ($bans = mysqli_fetch_array($kpidowf)): ?>
				{ label: "<?php echo $bans['Time'];?>", y: <?php echo $bans['Download']; ?> },
                          	<?php endwhile ?>
		...
		},
		{
			...
			toolTipContent: "<p style='color:purple'> {label}</p><hr/>Upload Speed: {y} mb/s",
			color: "green" ,
			dataPoints: [
				<?php while ($bans = mysqli_fetch_array($kpiupf)): ?>
				{ label: "<?php echo $bans['Time'];?>", y: <?php echo $bans['Upload']; ?> },
                          	<?php endwhile ?>
...
if (isset($_POST['filter'])) {
	...
	$nstartd = str_replace('-','/',$nstartd);
        ...
        $kpidow=mysqli_query($con,"SELECT * FROM kpidata WHERE Time LIKE '%$nstartd%' ORDER BY DATA_ID ASC");
        $kpiup=mysqli_query($con,"SELECT * FROM kpidata WHERE Time LIKE '%$nstartd%' ORDER BY DATA_ID ASC");
?>
...
window.onload = function () {
	var chart = new CanvasJS.Chart("chartContainer", {
		...
		data: [
		{
			...
			color: "purple" ,
			dataPoints: [
                                <?php while ($bans = mysqli_fetch_array($kpidow)): ?>
				{ label: "<?php echo $bans['Time'];?>", y: <?php echo $bans['Download']; ?> },
                          	<?php endwhile ?>
		...
		},
		{
			...
			color: "green" ,
			dataPoints: [
				<?php while ($bans = mysqli_fetch_array($kpiup)): ?>
				{ label: "<?php echo $bans['Time'];?>", y: <?php echo $bans['Upload']; ?> },
                          	<?php endwhile ?>
